...
| Linux bundle: `libOpenEXRCore.so.33: undefined symbol: libdeflate_alloc_compressor_ex` → SAM dead on Linux | bundled libdeflate older than what OpenEXR (linked by conda **vigra**) needs. Same class as the pillow/cv2 fixes: force one consistent copy | **still open** — likely fix mirrors the pillow step |
